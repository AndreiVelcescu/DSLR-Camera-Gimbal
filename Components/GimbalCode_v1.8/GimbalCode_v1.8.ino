...
void setup() {
  Serial.begin(115200);
  // Initialise Encoders_________________________ 
  setup_Encoders();
  // Set Timer Interupt Periods__________________
  setup_TimerInterrupts();
  //_________________PWM SetUp___________________
  ...
  // Osciloscope output - test pins______________
  pinMode(pin0, OUTPUT);
  pinMode(pin1, OUTPUT);
  pinMode(pin2, OUTPUT);

  // Setup Gryoscopes____________________________
  setup_gyro1();
  setup_gyro2();
  // ____________________________________________
...
volatile float i=0, prev_i=0, v_ref=0, ref=0, ref_1=0, ref_2=0, ref_3=0;
/////////////// Motor Control ////////////////////////////////////////////////////////////////
void motorControl(volatile int *u_array, volatile float reference, volatile float angle){
  ...
  // u = 20*(error - BNO2_v*30); // works well - with IMU not on the top of the test rig (snapy )
  // u = 30*(1.25*error - 50*BNO2_v); // works well 

  v_ref = 2*error + 25*i;
  u = 40*(v_ref - 40*dEnc); // works well (with signal genrator - added 57.29 - radians to degrees)
  abs_u = abs(u);
...
void ControlInt() {
  ...
  motorControl(u_array1,count,encAngle2);
  motorControl(u_array2,count,encAngle3);
  digitalWrite(pin2, LOW);
}

void derivativeInt(){
  integer_angle = encAngle2*10;
  dEnc = 100*(integer_angle - pre_Enc3)/(micros()-t_prev);
  dT = micros()-t_prev;
  t_prev = micros();
...
void loop() {
  ...
  Serial.print("\t");
  Serial.print(-referenceBNO-19);
  Serial.print("\t");
  Serial.print(dEnc);
  Serial.print("\t");
  Serial.println(-57.29*BNO_v);
  // Serial.print("\t");
  // Serial.print(encAngle2); 
  // Serial.print("\t");  
